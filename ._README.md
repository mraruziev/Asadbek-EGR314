...
<details>
<summary>Building the datasheet site locally</summary>

The site is MkDocs + the Material theme. Pushing to `main` triggers the
**mkdocs-build** action, which deploys it to GitHub Pages.
...
If a push doesn't publish: check the repository
[settings checklist](https://embedded-systems-design.github.io/fork-report-website/#settings-to-check/),
then rename `.github/workflows/pages.yml` to `main.yml`, commit and push — that
usually kicks the build back into life. Watch it run under the **Actions** tab.
