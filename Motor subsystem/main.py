...
# Demo runs once: triggered by the first Start broadcast from Jacob.
demo_running = False
...
# ---------------------------------------------------------------------------
# Demo sequence
# Triggered by the first Start broadcast from Jacob. Waits 10 s, then drives
# FORWARD 5 s, RIGHT 5 s, LEFT 5 s. The motor command is re-sent every 200 ms
# so the 350 ms failsafe can't stop it mid-leg.
# ---------------------------------------------------------------------------

async def _drive_for(action, label, duration_ms):
    print(">>> DEMO:", label, "for", duration_ms, "ms")
...
async def demo_sequence():
    ...
    print("\n" + "=" * 50)
    print(">>> DEMO ARMED - waiting 10 s before motion")
    print("=" * 50)
    await asyncio.sleep(10)

    await _drive_for(lambda: set_h_bridge_spi(True), "FORWARD", 5000)
    await _drive_for(motor_right,                    "RIGHT",   5000)
    await _drive_for(motor_left,                     "LEFT",    5000)
...
def process_motor_command(sender, data):
    """
    Returns (response_receiver, response_data) or None.
    """
    try:
        parts = data.split(":")
        ...
        # Start broadcast from Jacob - acknowledge directly to him AND
        # kick off the 10s-then-drive demo (only the first time).
        if len(parts) >= 3 and parts[0] == "ST" and parts[1] == "S":
            blink_green()
            print(">>> START MESSAGE RECEIVED FROM '%s'" % id_to_char(sender))
            if not demo_running:
                asyncio.create_task(demo_sequence())
            return HMI_ID, "ST:S:WheelReady;"
...
async def handle_packet(packet):
    ...
    if receiver == MY_ID or receiver == BCAST_ID:
        ...
        # Send the response over MQTT regardless of whether the original
        # was unicast or a broadcast - the Start ack to Jacob has to go out
        # even though he sent us a broadcast.
        if response and mqtt_client:
            response_receiver, response_data = response
            response_packet = build_packet(response_receiver, response_data)
            print(">>> Sending response via MQTT to '%s': %s" %
                  (id_to_char(response_receiver), response_data))
            await mqtt_client.publish(TOPIC_W_TO_C, response_packet, qos=1)

        # Forward broadcast down the UART chain
        if receiver == BCAST_ID:
    ...
    # Message for another subsystem - forward via UART
    print("\n" + ">" * 50)
...
async def main(cli):
    ...
    print("Demo: triggered on first ST:S:Start; from 'h' (Jacob)")
    print("=" * 70 + "\n")
